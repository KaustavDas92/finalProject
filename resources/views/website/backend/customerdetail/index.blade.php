                                    ...
                                    <div class="col-sm-4">
                                        <div id="datatable-responsive_filter" class="dataTables_filter">
                                            <label> Create a new Customer
                                                <form method="GET" action="{{route('customerdetail.create')}}">
                                                    @csrf
...
{{--                                                <th class="sorting" tabindex="0" aria-controls="datatable-responsive" rowspan="1" colspan="1" style="width: 65px;" aria-label="Last name: activate to sort column ascending">Action</th>--}}
                                            ...
                                            @foreach($customerDetails as $cd)
                                                <tr role="row" class="odd">
                                                    <td tabindex="0" class="sorting_1">{{$cd->fname}}</td>
                                                    <td>{{$cd->lname}}</td>
                                                    <td>{{$cd->address1}}</td>
                                                    <td>{{$cd->email}}</td>
                                                    <td class>
                                                        <a class="btn btn-app"
                                                           href="{{route('customerdetail.edit',$cd->id)}}">
                                                            <i class="fa fa-edit"></i>
                                                            Edit
                                                        </a>
                                                        <form method="POST"
                                                              action="{{route('customerdetail.destroy',$cd->id)}}">
                                                            @csrf
                                                            @method('DELETE')
