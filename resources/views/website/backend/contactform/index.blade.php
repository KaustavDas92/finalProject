                                    ...
                                    <div class="col-sm-4">
                                        <div id="datatable-responsive_filter" class="dataTables_filter">
                                            <label> Create a new Product
                                                <form method="GET" action="{{route('contactform.create')}}">
                                                    @csrf
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap dataTable no-footer dtr-inline collapsed" cellspacing="0" width="100%" role="grid" aria-describedby="datatable-responsive_info" style="width: 100%;">
                                            <thead>
                                            <tr role="row">
                                                <th class="sorting_asc" tabindex="0" aria-controls="datatable-responsive" rowspan="1" colspan="1" style="width: 66px;" aria-sort="ascending" aria-label="First name: activate to sort column descending">Message</th>
                                                <th class="sorting" tabindex="0" aria-controls="datatable-responsive" rowspan="1" colspan="1" style="width: 65px;" aria-label="Last name: activate to sort column ascending">User Name</th>
                                            ...
                                            @foreach($contactForms as $cf)
                                                <tr role="row" class="odd">
                                                    <td tabindex="0" class="sorting_1">{{$cf->message}}</td>
                                                    <td>{{$cf->username}}</td>
                                                    <td>{{$cf->email}}</td>
                                                    <td>{{$cf->subject}}</td>
                                                    <td class>
                                                        <a class="btn btn-app"
                                                           href="{{route('contactform.edit',$cf->id)}}">
                                                            <i class="fa fa-edit"></i>
                                                            Edit
                                                        </a>
                                                        <form method="POST"
                                                              action="{{route('contactform.destroy',$cf->id)}}">
                                                            @csrf
                                                            @method('DELETE')
