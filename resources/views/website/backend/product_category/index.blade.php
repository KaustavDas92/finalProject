                                    ...
                                    <div class="col-sm-4">
                                        <div id="datatable-responsive_filter" class="dataTables_filter">
                                            <label> Create a new Product Category
                                                <form method="GET" action="{{route('productcategory.create')}}">
                                                    @csrf
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap dataTable no-footer dtr-inline collapsed" cellspacing="0" width="100%" role="grid" aria-describedby="datatable-responsive_info" style="width: 100%;">
                                            ...
                                            <tbody>
                                            @foreach($productCategories as $brands)
                                                <tr role="row" class="odd">
                                                    <td tabindex="0" class="sorting_1">{{$brands->brand_name}}</td>
                                                    <td>{{$brands->status}}</td>
                                                    <td class="flex">
                                                        <a class="btn btn-app"
                                                           href="{{route('productcategory.edit',$brands->id)}}">
                                                            <i class="fa fa-edit"></i>
                                                            Edit
                                                        </a>
                                                        <form method="POST"
                                                              action="{{route('productcategory.destroy',$brands->id)}}">
                                                            @csrf
                                                            @method('DELETE')
